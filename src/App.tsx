...
function FloatingButtons() {
  ...
  return (
    <div className="fixed bottom-6 right-6 z-50 flex flex-col items-end gap-3">
      ...
      {chatOpen && (
        <div className="mb-1 w-80 rounded-2xl overflow-hidden shadow-2xl"
          style={{ background: 'var(--card)', border: '1px solid rgba(194,160,110,0.25)' }}>
          <div className="px-4 py-3 flex items-center justify-between"
            style={{ background: 'linear-gradient(135deg,#c2a06e,#8b6914)' }}>
            <div className="flex items-center gap-2">
              <div className="w-2 h-2 rounded-full bg-green-300 animate-pulse" />
              <span className="text-white font-bold text-sm">কাস্টমার সার্ভিস</span>
            </div>
            <button onClick={() => setChatOpen(false)} className="text-white/80 hover:text-white text-lg leading-none">✕</button>
          </div>
          <div className="p-4">
            <p className="text-sm mb-3" style={{ color: 'var(--text2)' }}>আমাদের সাথে যোগাযোগ করুন:</p>
            <a href={`https://wa.me/${WA}?text=শিল্পশপ থেকে সাহায্য দরকার`}
              target="_blank" rel="noopener noreferrer"
              className="flex items-center gap-3 p-3 rounded-xl mb-2 transition-all hover:opacity-90"
              ...
              <div>
                ...
                <p className="text-white/80 text-xs">সরাসরি চ্যাট করুন</p>
              </div>
            </a>
            <a href="/contact"
              className="flex items-center gap-3 p-3 rounded-xl transition-all hover:opacity-90"
              style={{ background: 'rgba(194,160,110,0.12)', border: '1px solid rgba(194,160,110,0.3)' }}>
              <svg className="w-5 h-5 shrink-0" style={{ color: '#c2a06e' }} fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                <path strokeLinecap="round" strokeLinejoin="round" d="M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z" />
              </svg>
              <div>
                <p className="font-bold text-sm" style={{ color: 'var(--accent)' }}>ইমেইল / ফর্ম</p>
                <p className="text-xs" style={{ color: 'var(--text3)' }}>বার্তা পাঠান</p>
              </div>
            </a>
          </div>
        </div>
      ...
      {/* Customer Service toggle button */}
      <button onClick={() => setChatOpen(v => !v)}
        title="কাস্টমার সার্ভিস"
        className="w-14 h-14 rounded-full flex items-center justify-center shadow-2xl transition-all hover:scale-110"
        style={{ background: chatOpen ? 'linear-gradient(135deg,#8b6914,#c2a06e)' : 'linear-gradient(135deg,#c2a06e,#8b6914)' }}>
        {chatOpen ? (
